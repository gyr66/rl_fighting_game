...
# from myenv.Machete import Machete
...
class FightingiceEnv(gymnasium.Env):
    ...
    def __init__(self, **kwargs):
        ...
        if self.system_name == "linux":
            ...
            print("Please make sure you can run java if you see some error", flush=True)
    ...
    def reset(self, p2="MctsAi", seed=None, options=None):  # BC P MctsAi
        ...
        if self.round_num >= self.freq_restart_java:
            # while True:
            # try:
            #     # print("self._close_gateway()")
            #     self._close_gateway()
            #     # print("self._close_java_game()")
            #     self._close_java_game()
            #     # print("self._start_java_game()")
            #     self._start_java_game()
            #     # break
            # except:
            #     # raise SystemExit("Can not restart game")
            #     print("Can not restart game")
    ...
    def step(self, action):
        ...
        if self.pipe.poll(60):
            ...
            print("can't receive signals within 60 seconds. let's terminate gym env.", flush=True)
        # return new_obs, reward, done, info
        # Need to be compatible with gymnasiun
...
# class FightingiceEnv(FightingiceEnvF):
...
#         # while True:
#         #     try:
#         #         new_obs, reward, done, _ = super().step(action)
#         #         if new_obs != None:
#         #             return new_obs, reward, done, {}
#         #         else:
#         #             new_obs = super().reset(self.p2)
#         #             return new_obs, 0, True, {}
#         #     except Exception:
#         #         pass
#         return super().step(action)

#     def reset(self, p2="MctsAi"):
#         # while True:
#         #     try:
#         #         new_obs = super().reset(p2)
#         #         if new_obs != None:
#         #             return new_obs
#         #     except Exception:
#         #         pass
